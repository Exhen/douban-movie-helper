...
// 
var updateSites = function (forceFlag) {
    lastUpdate = GM_getValue('lastUpdate', '');
    now = new Date();
    if (lastUpdate == '' || ((now.getTime() - parseInt(lastUpdate)) > 86400000) || forceFlag) {
        getJSON('https://raw.githubusercontent.com/Exhen/douban-movie-helper/master/sites.json', function (doc, url) {
            if (doc) {
